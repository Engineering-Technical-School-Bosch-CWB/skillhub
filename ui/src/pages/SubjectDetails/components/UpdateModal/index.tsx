            ...
            <Modal open={isOpen} handleClose={handleClose} title={`${subject.curricularUnit} - ${subject.class}`} >
                <div className={`${styles.inputs}`}>
                    <div className={`${styles.section}`}>
                        <Input onChange={(e) => setPayload({ ...payload, durationHours: Number(e.target.value) })} label="Duration hours" width={"35%"} value={payload.durationHours} type="number" min={0} />
                        <Input onChange={(e) => setPayload({ ...payload, period: Number(e.target.value) })} label="Period" width={"20%"} value={payload.period} type="number" min={0} max={50} />
                        <Input dateChange={(e) => setPayload({ ...payload, beganAt: e?.format("YYYY-MM-DD") })} label="Initial date" width={"45%"} value={payload.beganAt ? formatDate(payload.beganAt) : undefined} type="date" />
                    </div>
                    <Select
